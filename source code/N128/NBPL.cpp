...
bool x_uG(int* xinput1,int* xinput2,int N)
{
	bool value = true;
	int* x = new int[N];
	int n = (int)(log((double)N)/log(2.0));
	for (int i = 0; i < N; i++)
	{
		x[i] = xinput1[i];
	}
...
void BP_decoding(double* LLR,int * Out, int M, int * frozen,int iternum,int * poly,int crclen,int* frozen_ori,int * inx,int *flg)
{
	...
	int * xinput1 = new int[M];
	int * xinput2 = new int[M];
	...
	{	
		...
		for (int i = 0; i < M; i++)
		{
			...
			xinput1[i] = (L[0][i]+R[0][i]) >= 0 ? 0 : 1;
			xinput2[i] = (L[m][i]+R[m][i]) >= 0 ? 0 : 1;
		}
		...
		bool far = x_uG(xinput1, xinput2, M);	
		if (CFlag && far)
		{
			flg[0] = 0;
			flg[1] = ite;
